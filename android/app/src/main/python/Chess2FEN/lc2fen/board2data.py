...
from Chess2FEN.lc2fen.detectboard import detect_board
from Chess2FEN.lc2fen.fen import fen_to_board, rotate_board_fen2image
from Chess2FEN.lc2fen.split_board import split_square_board_image, split_board_image
...
def process_input_boards(data_path):
    ...
    for input_board in tqdm(input_boards):
        ...
        input_array = cv2.imread(input_board)
        image_object = detect_board.detect(input_array, output_board)
        _, square_corners = detect_board.compute_corners(image_object)

        name = os.path.splitext(os.path.basename(input_board))[0]
        split_board_image(input_array, square_corners, name,
                          data_path + '/pieces')
