...
from Chess2FEN.lc2fen.detectboard import debug, image_object
from Chess2FEN.lc2fen.detectboard import poly_point_isect

from os.path import dirname, join

from tflite_runtime.interpreter import Interpreter

__laps_model = join(dirname(__file__), 'models/laps.tflite')

# Load the TFLite model and allocate tensors.
LAPS_MODEL = Interpreter(model_path=__laps_model)
LAPS_MODEL.allocate_tensors()

# Get input and output tensors.
input_details = LAPS_MODEL.get_input_details()
output_details = LAPS_MODEL.get_output_details()
...
def __is_lattice_point(img):
    ...
    LAPS_MODEL.set_tensor(input_details[0]['index'], X)

    LAPS_MODEL.invoke()

    # The function `get_tensor()` returns a copy of the tensor data.
    # Use `tensor()` in order to get a pointer to the tensor.
    pred = LAPS_MODEL.get_tensor(output_details[0]['index'])[0]
