...
from Chess2FEN.lc2fen.detectboard import debug
from Chess2FEN.lc2fen.detectboard.cps import cps
from Chess2FEN.lc2fen.detectboard.image_object import ImageObject
from Chess2FEN.lc2fen.detectboard.laps import laps, check_board_position
from Chess2FEN.lc2fen.detectboard.slid import slid
...
def detect(input_array, output_board, board_corners=None):
    """
    Detects the board position in input_array and stores the cropped
    detected board in output_board.

    :param input_array: Input chessboard image.
    ...
    if board_corners is not None:
        found, cropped_img = check_board_position(input_array, board_corners)
        if found:
            cv2.imwrite(output_board, cropped_img)
            image = ImageObject(input_array)
