...
import numpy as np
from Chess2FEN.clipper.clipper import *
...
def __polyscore(cnt, pts, cen, alfa, beta):
    ...
    polys = [[Point(v[0], v[1]) for v in cnt]]

    # Too few points
    pco = OffsetPolyLines(polys, gamma, JoinType.Miter, EndType.Closed)
    pco = [[[v.x, v.y] for v in pco[0]]]

    pcnt = matplotlib.path.Path(pco[0])
...
def __padcrop(img, four_points):
    ...
    polys = [[Point(v[0], v[1]) for v in four_points]]

    # Too few points
    pco = OffsetPolyLines(polys, 60, JoinType.Miter, EndType.Closed)
    pco = [[[v.x, v.y] for v in pco[0]]]
